...
define(['jquery','underscore','moment','kendo','Blob','base64','jszip','FileSaver',
    ...
    App.formatOneTotals =  function(hier, costs,type) {
        ...
        console.time('Hierarchy Loop Filter');
        ...
        console.timeEnd('Hierarchy Loop Filter');
        console.time('Begin CPI SPI calc');
       $.each(parent, function (kk, vv) {
        ...
        console.timeEnd('Begin CPI SPI calc');
    ...
    App.FilterChartData = function(results) {
        ...
        if (_.isArray(BCWS)) {
            ...
            var BCWSdata = _.map(BCWS, function (value) {
                ...
                dateCheck =  moment(value.Date).isAfter(value.SnapshotDate);
                return {
                    ...
                    "Type":"BCWS",
                    "Date": value.Date,
                    "AfterSnapshot":dateCheck
                }
            });//convert IntValProjCurr key for Chart Series
            var bcwsTotal = 0, bcwsHrs = 0, curBcwsTotal = 0, curBcwsHrs = 0;
            $.each(BCWSdata, function (key, value) {
                bcwsTotal += parseFloat(value.BCWS);
                bcwsHrs += parseFloat(value.QuantityBCWS);
                if(!value.AfterSnapshot){
                    curBcwsTotal += parseFloat(value.BCWS);
                    curBcwsHrs += parseFloat(value.QuantityBCWS);
                }
            });
            master.totals.push({"bcwsTotal": App.Math.ceil10(bcwsTotal, -2),"bcwsHrs":App.Math.ceil10(bcwsHrs, -2),"curBcwsTotal": App.Math.ceil10(curBcwsTotal, -2),"curBcwsHrs":App.Math.ceil10(curBcwsHrs, -2)});//.toFixed(2)
        ...
        if (_.isArray(EAC)) {
            ...
            var EACdata = _.map(EAC, function (value) {
                ...
                dateCheck =  moment(value.Date).isAfter(value.SnapshotDate);
                //  console.log(runningTotalEAC.toFixed(2) +' '+ value.IntValProjCurr);
                return {
            ...
            var eacTotal = 0, eacHrs = 0, curEacTotal = 0, curEacHrs = 0;
            $.each(EACdata, function (key, value) {
                eacTotal += parseFloat(value.EAC);
                eacHrs += parseFloat(value.QuantityEAC);
                if(!value.AfterSnapshot){
                    curEacTotal += parseFloat(value.EAC);
                    curEacHrs += parseFloat(value.QuantityEAC);
                }
            });
            console.log('eacTotal '+eacTotal);
            master.totals.push({"eacTotal": App.Math.ceil10(eacTotal, -2),"eacHrs": App.Math.ceil10(eacHrs, -2),"curEacTotal": App.Math.ceil10(curEacTotal, -2),"curEacHrs":  App.Math.ceil10(curEacHrs, -2)});//.toFixed(2)
        }
        if (_.isArray(ACWP)) {
        ...
        if (ETC != undefined || '') {
            var ETCdata = _.map(ETC, function (value) {
                dateCheck =  moment(value.Date).isBefore(value.SnapshotDate);
                return {"ETC": Number(value.IntValProjCurr),"SnapshotDate":dateCheck};
       ...
       // console.log(master.totals);
        console.timeEnd('FilterChartData');
        return master;
    };
